...
12. [DOC_REVIEW_PROFILE.md](docs/review/DOC_REVIEW_PROFILE.md)
13. [CODEX_REVIEW_PROFILE.md](docs/review/CODEX_REVIEW_PROFILE.md)
14. [REVIEW_SYSTEM.md](docs/review/REVIEW_SYSTEM.md)
...
- テストカバレッジは `95%` 以上を維持し、できれば `100%` を保つこと
- テストの `describe`、`test`、説明文は原則日本語で書くこと
- テスト工程が完了したら、必ずレビューを行うこと
- ブラウザ表示を伴う変更後は `mix browser.check` を行い、保存されたスクリーンショットも確認すること
- 新規または変更した関数には、短いコメントを付けること
- 公開関数には必ず `@doc` を付けること
