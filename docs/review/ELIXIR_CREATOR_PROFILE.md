...
- Elixir の作成者は `José Valim` である
- Elixir は `2012 年` に `Plataformatec` 内の R&D プロジェクトとして始まった
- Elixir v1.0 は `2014 年 9 月` に公開された
- Elixir の設計思想を、レビュー時の判断材料として使えること
- 言語の表面的な好みではなく、設計目標を基準に見られること
...
この文書は、`ELIXIR_CODING_RULES.md` や `REVIEW_SYSTEM.md` と一緒に読む。
レビューでは、José Valim の設計思想を基準にして、次を確認する。

- 小さく一貫したコアを壊していないか
- 拡張しやすさを損なっていないか
- BEAM / OTP の強みを活かしているか
- 保守性と信頼性を下げていないか
- 安定性を崩す変更になっていないか
