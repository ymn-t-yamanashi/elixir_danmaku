...
- 起動時に entrypoint で named volume の所有者を `app` に揃える
- `MIX_ENV` は用途ごとに切り替え、開発時は `dev`、検証時は `test` を使う

## 実装ルート

- `docker-compose.yml` の `app` サービスは、`.env` の `LOCAL_UID` と `LOCAL_GID` を受け取って起動ユーザーを合わせる
- 具体的には `user: "${LOCAL_UID:-1000}:${LOCAL_GID:-1000}"` の形を基本にする
